              ...
              <div key={m.id} className="bg-white rounded-2xl border border-stone-100 shadow-sm p-5 flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-muted">
                    ...
                    <span className="font-medium text-primary-light">{Array.isArray(m.department) ? m.department.join(", ") : m.department}</span>
                ...
                <form onSubmit={handleSaveEdit} className="p-6 space-y-4">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="block text-xs font-semibold text-primary uppercase tracking-wide">Department(s)</label>
                      <div className="grid grid-cols-2 gap-2 border border-stone-200 rounded-xl p-3 bg-stone-50">
                        {['Senior Choir', 'Youth Choir', "Children's Choir", 'Instrumentalists', 'Media Team', 'Sound/Technical'].map(dept => (
                          <label key={dept} className="flex items-center gap-2 cursor-pointer">
                            <input
                              type="checkbox"
                              checked={Array.isArray(editing.department) ? editing.department.includes(dept) : editing.department === dept}
                              onChange={(e) => {
                                const current = Array.isArray(editing.department) ? editing.department : (editing.department ? [editing.department] : []);
                                if (e.target.checked) {
                                  setEditing(prev => prev ? ({ ...prev, department: [...current, dept] }) : null);
                                } else {
                                  setEditing(prev => prev ? ({ ...prev, department: current.filter(d => d !== dept) }) : null);
                                }
                              }}
